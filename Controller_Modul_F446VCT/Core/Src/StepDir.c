...
//#define TIMER_INTERRUPT TIM3_IRQHandler
...
/*
	...
	*/
	...
	for (uint8_t ch = 0; ch < STEP_DIR_CHANNELS; ch++)
	{
		...
		HAL_GPIO_WritePin(currCh->stepPin->GPIOx, currCh->stepPin->GPIO_Pin, 0);
		...
		{
			//HAL_GPIO_ReadPin(*currCh->stallGuardPin->GPIOx, *currCh->stallGuardPin->GPIO_Pin) == GPIO_PIN_SET;
			StepDir_stallGuard(ch, HAL_GPIO_ReadPin(*currCh->stallGuardPin->GPIOx, *currCh->stallGuardPin->GPIO_Pin) == GPIO_PIN_SET );
		...
		*/
		...
		// Direction
		HAL_GPIO_WritePin(currCh->dirPin->GPIOx, currCh->dirPin->GPIO_Pin, (dx > 0) ? GPIO_PIN_SET : GPIO_PIN_RESET);
		...
		HAL_GPIO_WritePin(currCh->stepPin->GPIOx, currCh->stepPin->GPIO_Pin, GPIO_PIN_SET);
...
void StepDir_init(uint32_t precision)
{
	...
	for (int i = 0; i < STEP_DIR_CHANNELS; i++)
	{
		...
		StepDir[i].stallGuardPin        = NULL;
		StepDir[i].stepPin              = NULL;
		StepDir[i].dirPin               = NULL;
